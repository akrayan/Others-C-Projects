...
** Last update Mon Feb  6 12:56:22 2017 Axel
*/
...
{
  if (  !((int)((((char *)pmetadata - (char *)pblock) % pagesize) + sizeof(t_list) + s) < pagesize) )
    {
      write(1, "Depasse pagesize\n", strlen("Depasse pagesize\n"));
      if (sbrk( (((sizeof(t_list) + s) / pagesize) + 1) * pagesize ) == (void *)-1)
	return (NULL);
    }
  return ((void *)2);
}
...
void	*malloc2(size_t s)
{
  ...
  if (list == NULL)
      ...
      if (tmp != NULL)
	{
	  ...
	  pmetadata = tmp->block;
	  if (tmp->size > s)
	    add_after_elem((char *)tmp->next - ((int)pmetadata % pagesize) ,
			   (char *)pmetadata + s, (char *)pmetadata + s + sizeof(t_list), &tmp);
	  tmp->size = s;
	  return (pmetadata);
	}
      else
