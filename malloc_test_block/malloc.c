...
** Last update Mon Feb  6 17:36:38 2017 Axel
*/
...
void	*malloc2(size_t s)
{
  ...
  if (list == NULL)
      ...
      else
	{
	  ...
	  putstr("alloooooo2\n");
	  add_elem(s, pmetadata, (char *)pmetadata + sizeof(t_list));
	  if ((int)(((char *)pmetadata + sizeof(t_list) + s - (char *)pblock)) % pagesize != 0)
	    {
	      putstr("alloooooo3\n"); 
	      add_elem(pagesize - (((char *)pmetadata + s + sizeof(t_list) - (char *)pblock) % pagesize),
		       (char *)pmetadata + sizeof(t_list) + s, (char *)pmetadata + (2 * sizeof(t_list)) + s);
	      last->is_free = 1;
	    }
	}
    }
