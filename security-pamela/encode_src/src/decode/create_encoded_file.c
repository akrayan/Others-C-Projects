...
** Last update Wed Nov 29 18:02:32 2017 BIllel
*/
...
void	create_directory(t_elem *file_names)
{
  ...
  int		long_size_dir = get_long_size(tmp);
...
void	write_in_file_encode(t_file *myFile)
{
  int		byte = 0;
  t_cryptmsg	myMsg;
  char		*res;
  ...
  while ((byte = read(myFile->old_fd, myFile->data, READ_BUFFER_SIZE - 1)) > 0)
    {
      //myFile->data[byte] = 0;
      myMsg.msg = myFile->data;
      myMsg.len = byte;

      //printf("data = %s taille = %d\n", myFile->data, byte);
      res = decryption(myMsg);
      printf("akkache sue moi -%s-\n", res);
      if ((write(myFile->new_fd, res, strlen(res))) == -1)
	perror("write");
    }
  write(1, "finished...\n", 12);
}
...
void	create_file_encoded(t_elem *file_names)
{
  t_elem	*tmp = file_names;
  t_file	*myFile;
  int		long_size = get_long_size(file_names);
